...
> ⚠️ **Важливо**: Vercel безкоштовний план має ліміт 10 секунд для serverless functions. Для повного функціоналу (генерація + деплоймент займає 30-60 сек) використовуйте локальний запуск!

### Локальний запуск (Рекомендовано) {#local-setup}
...
# 2. Налаштувати .env (скопіюйте ваші токени)
cp .env.example .env
# Відредагуйте .env файл, додайте:
...
# Backend запущено на http://localhost:3002

# 5. У новому терміналі запустити UI
cd ui-simple
python3 -m http.server 30002
# UI доступний на http://localhost:30002
```

**Готово!** Відкрийте http://localhost:30002 і створюйте застосунки без обмежень по timeout! 🎉
